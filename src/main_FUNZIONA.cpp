...
const char* ssid = "iPhone di Michele";//"Andrea's Galaxy S24";
const char* password = "Michele4!";//"cdjb0132";
...
void TaskProcessFFT(void *pvParameters) {
  for(;;) {
    ...
    while(fft_index < samples) { vTaskDelay(pdMS_TO_TICKS(10)); }
...
void TaskLoRa(void *pvParameters) {
  ...
  for(;;) {
    ...
    if(node.isActivated()) {
      Serial.println("[LORA] Invio dato a TTN...");
      uint8_t payload[4];
      ...
      if(tx_state == RADIOLIB_ERR_NONE || tx_state == -1116 || tx_state == RADIOLIB_ERR_RX_TIMEOUT) {
         Serial.println("[LORA] Pacchetto INVIATO con successo a TTN!");
      } else {
         Serial.printf("[LORA] Errore invio: %d\n", tx_state);
      }
    }
  }
}
...
// ------------ MAIN SETUP ------------
void setup() {
  // Alimentazione hardware radio (Vext)
  pinMode(36, OUTPUT);
  digitalWrite(36, LOW); 
  delay(100);
  ...
  // Assegnato al Core 0 per non ostacolare il WiFi/Sensor
